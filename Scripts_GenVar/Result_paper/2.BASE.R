...
for(year in 21){ #Change to any number of desired years
  ...
  source("./input_sim/AdvanceYearGS.R") #Advances yield trials by a year
  ...
  save(stage.year.Parents, stage.year.F1, stage.year.HDRW, stage.year.PYT,
       stage.year.AYT, stage.year.EYT, file = paste0("./output_sim/stages_year",year,".RData"))
...
length(trainPop@id); head(trainPop@id)
length(trainPop@pheno); head(trainPop@pheno)
length(trainPop@fixEff); head(trainPop@fixEff)
